-- =============================================
-- 模型名称：dws_ranch_cattle_growth_agg_wi
-- 模型描述：牧场牛只生长周统计表，按自然周统计牛只生长指标
-- Dbt更新方式：增量（按自然周）
-- 粒度：牧场 + 栏舍 + SKU + 自然周
-- 说明：
--   - 数据源：dws_ranch_cattle_weigh_agg_i（牛只日快照）
--   - 增量策略：按自然周追加
--   - 统计指标：ADG、增重、体重分布等生长指标
--   - 聚合逻辑：按牧场+栏舍+SKU+自然周聚合牛只日快照，计算体重统计、ADG分布、月龄分布等
-- =============================================
{{ config(
    materialized='incremental',
    incremental_strategy='append',
    partition_by=['natural_week'],
    description='牧场牛只生长周统计表，按自然周统计牛只的生长指标（ADG、增重、体重分布等）',
    tags=['ranch', 'dws', 'agg', 'cattle', 'growth', 'weekly']
) }}
...
growth_weekly AS (
    SELECT
        ...
        -- 时间维度
        -- ====================
        MIN(stats_date) AS week_start_date,
        MAX(stats_date) AS week_end_date,
        COUNT(DISTINCT stats_date) AS valid_days,

        -- ====================
        -- 数量统计
        -- ====================
        COUNT(DISTINCT cattle_id) AS total_cattle_count,        -- 本周过牛只总数
        COUNT(DISTINCT CASE WHEN current_weight IS NOT NULL THEN cattle_id END) AS weighed_cattle_count,  -- 本周称重牛只数
...
-- {% if is_incremental() %}
-- WHERE natural_week > (SELECT COALESCE(MAX(natural_week), 0) FROM {{ this }})
-- {% endif %}

ORDER BY natural_week DESC, ranch_id, stall_id, cattle_sku_id
