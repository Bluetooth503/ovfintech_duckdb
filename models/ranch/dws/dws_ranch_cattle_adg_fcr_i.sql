-- =============================================
-- 模型名称：dws_ranch_cattle_adg_fcr_i
-- 模型描述：牛只ADG区间汇总表（增量），计算区间ADG和料肉比基础指标
-- Dbt更新方式：增量（事件级）
-- 粒度：每头牛每个称重日一条记录（每天去重，保留最后一次）
-- 说明：
--   - 数据源：dwd_ranch_cattle_weight_fact_i（DWD层称重明细）+ dwd_ranch_cattle_feed_fact_i（DWD层饲料明细）+ dim_ranch_cattle（牛只维度表）+ dim_ranch_grow_stage（生长阶段维度表）
--   - 增量策略：事件级追加
--   - 统计指标：区间ADG、区间料肉比、区间增重、整体ADG、累计增重、生长阶段匹配等指标
--   - 聚合逻辑：使用窗口函数计算区间增重和日增重，关联饲料数据计算料肉比，按品种+体重区间匹配生长阶段
-- =============================================
{{ config(
...
calc_interval_metrics AS (
    SELECT
        ...
        -- 窗口函数：计算区间日均增重（ADG）
        CASE WHEN LAG(weight_date) OVER (PARTITION BY cattle_id ORDER BY weight_date) IS NOT NULL THEN (weight - LAG(weight) OVER (PARTITION BY cattle_id ORDER BY weight_date)) / NULLIF(DATE_DIFF('day', LAG(weight_date) OVER (PARTITION BY cattle_id ORDER BY weight_date), weight_date), 0) ELSE NULL END AS period_adg,
        -- 称重事件序号（第几个有效称重日）
...
join_base AS (
    SELECT
        ...
        -- 整体ADG（从入栏至今）
        CASE WHEN c.in_stall_date IS NOT NULL AND m.current_weight_date > c.in_stall_date THEN (m.current_weight - c.in_stall_weight) / NULLIF(DATE_DIFF('day', c.in_stall_date, m.current_weight_date), 0) ELSE NULL END AS overall_adg,
        -- 上次称重日期（用于关联饲料数据）
        m.prev_weight_date
...
join_growth_stage AS (
    SELECT
        ...
        gs.stage_days
    FROM join_base b
    LEFT JOIN lkp_grow_stage gs ON b.sku_id::VARCHAR = gs.sku_id::VARCHAR AND b.current_weight >= gs.stage_start_weight AND b.current_weight < gs.stage_end_weight
),
