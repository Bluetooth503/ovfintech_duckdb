-- =============================================
-- 模型名称：dws_ranch_ai_inventory_agg_1d_d
...
-- 粒度：牧场 + 栏舍 + 日期
-- 说明：
--   - 数据源：dwd_ranch_region_ai_inventory_trx_i（DWD层AI盘点明细）+ dim_ranch_stall（栏舍维度表）
--   - 增量策略：按日期追加
...
WITH region_inventory AS (
    ...
    SELECT
        ...
        ranch_id,
        stats_date                          -- 统计日期
    FROM {{ ref('dwd_ranch_region_ai_inventory_trx_i') }}
