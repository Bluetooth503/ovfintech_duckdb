-- =============================================
-- 模型名称：dws_ranch_cattle_balance_agg_di
-- 模型描述：在栏牛只日统计表，按日期统计牧场/栏舍维度在栏牛只情况
-- Dbt更新方式：增量（按日期）
-- 粒度：牧场 + 栏舍 + SKU + 客户 + 日期
-- 说明：
--   - 数据源：dwd_ranch_cattle_onstall_fact_di（DWD层在栏明细）+ dim_ranch_cattle（牛只维度表）+ dim_ranch（牧场维度表）+ dim_ranch_stall（栏舍维度表）+ dim_ranch_sku（SKU维度表）
--   - 增量策略：按日期追加
--   - 统计指标：在栏数量、总重量、平均体重、总增重、预估价值、贷款金额、饲料成本、质押率等资产指标
--   - 聚合逻辑：取每头牛最新快照数据，按维度聚合统计
-- =============================================
{{ config(
...
cattle_state AS (
    ...
    SELECT
        ...
        s.total_loan_money,
        -- 计算预估价值：当前体重 * 实际单价
        CASE WHEN s.current_weight > 0 AND s.real_price > 0 THEN s.current_weight * s.real_price ELSE NULL END AS estimated_value,
        s.total_feed_cost,
        s.is_loan,
        s.if_sick,
...
balance_agg AS (
    SELECT
        ...
        -- 计算字段
        CASE WHEN SUM(install_weight) > 0 THEN SUM(weight_add) / SUM(install_weight) * 100 ELSE NULL END AS weight_add_ratio,                           -- 增重率(%)

        CASE WHEN SUM(weight_add) > 0 AND SUM(total_feed_cost) > 0 THEN SUM(total_feed_cost) / SUM(weight_add) ELSE NULL END AS feed_cost_per_kg,                           -- 每公斤增重成本
...
SELECT
    ...
    -- 质押率
    CASE WHEN b.total_estimated_value > 0 AND b.total_loan_money > 0 THEN ROUND(b.total_loan_money / b.total_estimated_value * 100, 2) ELSE NULL END AS pledge_ratio,
