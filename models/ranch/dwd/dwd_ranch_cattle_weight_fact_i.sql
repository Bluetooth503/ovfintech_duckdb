-- =============================================
-- 模型名称：dwd_ranch_cattle_weight_fact_i
-- 模型描述：牧场域牛只体重测量事务明细表，记录牛只体重测量的原子事件
-- Dbt更新方式：增量（事件级）
-- 粒度：id
-- 说明：
--   - 数据源：ods_ranch_weight（称重表），入栏/出栏体重作为补充称重记录注入
--   - 增量策略：按 id 增量追加
-- =============================================
{{ config(
...
src_install_weight AS (
    SELECT
        ...
        p.create_time,
        p.update_time
    FROM {{ ref('dwd_ranch_cattle_purchase_fact_i') }} p
...
src_outstall_weight AS (
    SELECT
        ...
        o.create_time,
        o.update_time
    FROM {{ ref('dwd_ranch_cattle_outstall_fact_i') }} o
