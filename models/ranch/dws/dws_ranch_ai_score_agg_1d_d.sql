-- =============================================
-- 模型名称：dws_ranch_ai_score_agg_1d_d
...
-- 粒度：牧场 + 栏舍 + 日期
-- 说明：
--   - 数据源：dwd_ranch_cattle_ai_score_trx_i + dws_ranch_cattle_snapshot_1d_d
--   - 增量策略：按日期追加
--   - 统计指标：AI评分、体况评分、等级分布等AI指标
-- =============================================
{{ config(
...
WITH ai_score_trx AS (
    ...
    FROM {{ ref('dwd_ranch_cattle_ai_score_trx_i') }}
