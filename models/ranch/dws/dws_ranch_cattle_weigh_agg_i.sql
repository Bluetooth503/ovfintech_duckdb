-- =============================================
-- 模型名称：dws_ranch_cattle_weigh_agg_i
-- 模型描述：牧场牛只称重事件汇总宽表，整合牛只多维度指标
-- Dbt更新方式：增量（事件级）
-- 粒度：每头牛每个称重日期一条记录
-- 说明：
--   - 数据源：dwd_ranch_cattle_weight_fact_i（DWD层称重明细）+ dws_ranch_cattle_adg_fcr_i（ADG汇总）+ dwd_ranch_cattle_price_fact_i（DWD层价格明细）+ dim_ranch_cattle（牛只维度表）
--   - 增量策略：事件级追加
--   - 统计指标：体重、月龄、品种、ADG、价格、在栏月龄区间、体重区间等多维度指标
--   - 聚合逻辑：关联称重、牛只维度、ADG、价格数据，整合为每头牛每个称重事件一条汇总记录
-- =============================================
{{ config(
    materialized='incremental',
    incremental_strategy='append',
    partition_by=['stats_date'],
    description='牧场牛只称重事件汇总宽表，整合牛只的体重、月龄、品种、ADG、价格等多维度指标',
...
integrated_data AS (
    ...
    LEFT JOIN price_data p ON w.stats_date = p.stats_date AND c.ranch_id::VARCHAR = p.ranch_id::VARCHAR AND c.cattle_sku_id::VARCHAR = p.cattle_sku_id::VARCHAR
)
