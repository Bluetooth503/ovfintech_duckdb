-- =============================================
-- 模型名称：dws_ranch_stall_performance_agg_di
-- 模型描述：栏舍绩效日统计表，按日期统计栏舍的运营绩效指标
-- Dbt更新方式：增量（按日期）
-- 粒度：牧场 + 栏舍 + 日期
-- 说明：
--   - 数据源：dws_ranch_cattle_weigh_agg_i + dws_ranch_cattle_adg_fcr_i
--   - 增量策略：按日期追加
--   - 统计指标：ADG、料肉比、增重率、周转率等绩效指标
--   - 聚合逻辑：按牧场+栏舍+日期聚合牛只快照与ADG/FCR数据，计算体重、ADG、料肉比、增重率等绩效统计
-- =============================================
{{ config(
