-- =============================================
-- 模型名称：dws_ranch_cattle_sell_agg_di
-- 模型描述：牛只销售日统计表，按日期统计牧场牛只销售情况
-- Dbt更新方式：增量（按日期）
-- 粒度：牧场 + 日期
-- 说明：
--   - 数据源：dwd_ranch_cattle_sell_fact_i（DWD层销售明细）+ dim_ranch（牧场维度表）
--   - 增量策略：按日期追加
--   - 统计指标：销售数量、总重量、平均重量、销售金额、平均单价、买家数等销售指标
--   - 聚合逻辑：按牧场+日期聚合销售明细，统计数量、重量、金额及出栏类型分布
--   - 出栏类型：正常销售、死亡、丢失
-- =============================================
{{ config(
