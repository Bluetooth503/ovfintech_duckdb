...
| `dws` 汇总层 | `state` 状态 | `_state_df` 日状态 | 当前最新状态，日全量覆盖 | `dws_fund_customer_loan_balance_state_df.sql` |
| | `agg` 聚合 | `_di` 日增量 | 按日期追加 | `dws_ranch_cattle_balance_agg_di.sql` |
| | | `_df` 日全量 | 按日期全量刷新（保留历史） | `dws_ranch_cattle_return_agg_df.sql` |
...
| | `snap` 快照 | `_snap_df` 日快照 | 日度历史快照（保留历史） | `dws_ranch_cattle_snap_df.sql` |
| | | `_snap_mf` 月快照 | 月度历史快照（保留历史） | `dws_ranch_cattle_inventory_snap_mf.sql` |
...
- 特殊后缀：
  - `_state_df`：当前状态快照（日全量覆盖，不保留历史）
  - `_snap_df`/`_snap_mf`：历史快照（保留历史数据）
  - `_profile`：画像，`_hist`：历史，`_cum_d`：累计日，`_cum_m`：累计月
...
dws_ranch_cattle_return_agg_df.sql        # 日全量（保留历史）
dws_ranch_cattle_growth_agg_mi.sql        # 月增量
dws_ranch_cattle_install_agg_mf.sql       # 月全量
dws_ranch_cattle_snap_df.sql              # 日快照（保留历史）
dws_ranch_cattle_inventory_snap_mf.sql    # 月快照（保留历史）
dws_ranch_cattle_adg_fcr_agg_i.sql        # 事件级
dws_ranch_cattle_price_state_df.sql       # 当前价格状态快照（不保留历史）
dws_ranch_cattle_weight_state_df.sql      # 当前称重状态快照（不保留历史）
dws_fund_customer_loan_balance_state_df.sql  # 当前状态快照（不保留历史）
